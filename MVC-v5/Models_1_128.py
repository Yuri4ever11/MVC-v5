...
class C7_DenseNet161(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # Modify the first convolution layer to match your input channels
        self.features.conv0 = nn.Conv2d(1, 96, kernel_size=7, stride=2, padding=3, bias=False)
...
class C7_DenseNet201(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(1920, num_classes)  # Output size of DenseNet-201 is 1920
...
class C8_MobileNetV2(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # Modify the first convolution layer to accept single channel input
        self.mobilenetv2.features[0][0] = nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1, bias=False)  # Adjust the input channels
...
class C11_IGCNet(nn.Module):
    def __init__(self, num_classes=4):
        super(C11_IGCNet, self).__init__()
        # Modify the first IGCNetBlock to accept single-channel input
        self.conv1 = IGCNetBlock(1, 64)  # Changed in_channels to 1
        self.conv2 = IGCNetBlock(192, 128)
        self.conv3 = IGCNetBlock(384, 256)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        # Adjust the in_features to match the output dimension of the IGCNetBlock layers
        self.fc = nn.Linear(768, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        x = self.conv3(x)
        print(x.shape)  # Print the shape after conv3
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
...
class C12_Darknet53(nn.Module):
    ...
    def make_layers(self, in_channels, out_channels, blocks):
        ...
        layers.append(DarknetBlock(in_channels, out_channels))
        for _ in range(1, blocks):
            # Keep the out_channels consistent for the subsequent DarknetBlocks
