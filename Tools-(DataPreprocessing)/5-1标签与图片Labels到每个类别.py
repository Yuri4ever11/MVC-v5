...
for label_file in label_files:
    ...
    if len(categories) == 1:
        ...
        dst_image_path = os.path.join(os.path.join(images_folder, str(category)), os.path.basename(image_path))
        ...
        if not os.path.exists(dst_image_path):
            shutil.move(image_path, os.path.join(images_folder, str(category)))
        else:
        ...
        dst_label_path = os.path.join(dst_folder, os.path.basename(label_path))
        dst_image_path = os.path.join(os.path.join(images_folder, 'multi_class'), os.path.basename(image_path))
        if not os.path.exists(dst_label_path):
            shutil.move(label_path, dst_folder)
        else:
            print(f"Skipping {label_file} as the destination file already exists.")
        if not os.path.exists(dst_image_path):
            shutil.move(image_path, os.path.join(images_folder, 'multi_class'))
        else:
            print(f"Skipping {image_name} as the destination file already exists.")