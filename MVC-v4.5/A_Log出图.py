...
def extract_info(log_path):
    ...
    for match in re.finditer(r'Test Accuracy for (\w+): ([\d.]+)%', content):
        label, acc = match.group(1), match.group(2)
        test_accuracies.setdefault(label, []).append(float(acc))

    avg_test_accuracy_match = re.search(r'Average Test Accuracy: (\d+\.\d+)', content)
    avg_test_accuracy = avg_test_accuracy_match.group(1) if avg_test_accuracy_match else None
    ...
    test_loss = [float(match.group(1)) for match in re.finditer(r'Test Loss: (\d+\.\d+)', content)]

    return epochs, train_accuracies, avg_train_accuracy, val_accuracies, avg_val_accuracy, test_accuracies, avg_test_accuracy, train_loss, val_loss, test_loss


# 生成子图并保存
def generate_plots(log_path):
    epochs, train_accuracies, avg_train_accuracy, val_accuracies, avg_val_accuracy, test_accuracies, avg_test_accuracy, train_loss, val_loss, test_loss = extract_info(log_path)
    ...
    # 测试集精度和平均精度
    for label, acc_values in test_accuracies.items():
        axs[2].plot(epochs[:len(acc_values)], acc_values, label=label)

    avg_test_values = [sum(values) / len(values) for values in zip(*test_accuracies.values())]
    axs[2].plot(epochs[:len(avg_test_values)], avg_test_values, marker='o', label='Average', linestyle='--')
    axs[2].set_title('Test Set Accuracy')
    axs[2].set_xlabel('Epoch')
    axs[2].set_ylabel('Accuracy (%)')
    axs[2].set_ylim(20, 100)  # 设置Y坐标轴范围
    axs[2].legend()

    # 损失图
    min_length_all = min(len(epochs), len(train_loss), len(val_loss), len(test_loss))
    axs[3].plot(epochs[:min_length_all], train_loss[:min_length_all], label='Training Loss')
    axs[3].plot(epochs[:min_length_all], val_loss[:min_length_all], label='Validation Loss')
    axs[3].plot(epochs[:min_length_all], test_loss[:min_length_all], label='Test Loss')
    axs[3].set_title('Loss')
    axs[3].set_xlabel('Epoch')
    axs[3].set_ylabel('Loss')
    # axs[3].set_ylim(0, 100)  # 设置Y坐标轴范围
    axs[3].legend()
